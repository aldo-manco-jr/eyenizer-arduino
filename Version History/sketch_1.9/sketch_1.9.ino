/*
 * Eyenizer 1.9 - Aldo & Eduardo Manco
...
void setup() 
{
  ...
  if(!SD.begin(0)) //Se il PIN CS Non è Impostato su 0
  {
    return;
  }
}
...
void loop() 
{
  ...
  //Se la Variabile Contiene il UID Tag del Badge di Cardarelli Ludovico
  if (badge.substring(1) == "10 9E 90 7E")
  {
     // Crea File Per Ogni Dipendente
     File ludovico;
     // Apri File Per Scrittura
     ludovico = SD.open("cardarlu.txt", FILE_WRITE);
     ...
     nCardarelliLudovico = daytime(ludovico, nCardarelliLudovico, minute, hour, dayOfWeek, dayOfMonth, month, year);
  ...
  {
    File emiliano;
    emiliano = SD.open("maniliem.txt", FILE_WRITE);
    nManiliEmiliano = daytime(emiliano, nManiliEmiliano, minute, hour, dayOfWeek, dayOfMonth, month, year);
    emiliano.close();
    lcd.clear();
    lcd.print("Manili Emiliano");
    employee(minute, hour, dayOfWeek, dayOfMonth, month, year);
  }
